...
contract B20Test is B20FactoryTest {
    ...
    /// filtering with the token's own address. Crediting the token
    ...
    function _knownPolicyType(uint8 idx) internal pure returns (bytes32) {
        ...
        if (i == 3) return B20Constants.SEIZE_EXEMPT_POLICY;
        if (i == 4) return B20Constants.SEIZE_RECEIVER_POLICY;
    ...
    function _isKnownPolicyType(bytes32 policyType) internal pure returns (bool) {
        return policyType == B20Constants.TRANSFER_SENDER_POLICY || policyType == B20Constants.TRANSFER_RECEIVER_POLICY
            || policyType == B20Constants.TRANSFER_EXECUTOR_POLICY || policyType == B20Constants.SEIZE_EXEMPT_POLICY
            || policyType == B20Constants.SEIZE_RECEIVER_POLICY || policyType == B20Constants.MINT_RECEIVER_POLICY;
    }
