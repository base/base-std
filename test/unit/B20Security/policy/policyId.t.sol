...
contract B20SecurityPolicyIdTest is B20SecurityTest {
    /// @notice Verifies policyId still resolves the four base policy types on the security variant.
    /// @dev The security variant's `policyId` resolution path must still terminate in the base
