...
contract B20SecurityUpdatePolicyTest is B20SecurityTest {
    /// @notice Verifies updatePolicy still routes base policy types through the inherited write path on the security variant.
    /// @dev The security variant adds no extra policy slots of its own; this is a sanity check that
