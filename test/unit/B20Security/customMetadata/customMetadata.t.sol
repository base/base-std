...
contract B20SecurityCustomMetadataTest is B20SecurityTest {
    ...
    ///      entry at creation, so ISIN reads as empty too on a fresh token.
    function test_customMetadata_success_emptyForUnset() public view {
        assertEq(security().customMetadata(IDENTIFIER_ISIN), "", "unset entry must read as empty string");
        assertEq(security().customMetadata(IDENTIFIER_CUSIP), "", "unset entry must read as empty string");
        assertEq(security().customMetadata(IDENTIFIER_FIGI), "", "unset entry must read as empty string");
    }
    ...
    function test_customMetadata_success_returnsStoredValue(string calldata value) public {
        ...
        assertEq(security().customMetadata(IDENTIFIER_CUSIP), value, "getter must return the last written value");
    }
}
