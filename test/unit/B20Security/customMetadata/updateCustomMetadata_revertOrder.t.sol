...
/// @title Differential check-order tests for `updateCustomMetadata`.
///
...
contract B20SecurityUpdateCustomMetadataRevertOrderTest is B20SecurityTest {
    ...
    /// @dev Caller lacks METADATA_ROLE AND the identifierType is empty.
    ///      Role modifier fires before the body's empty-type check.
    function test_updateCustomMetadata_revertOrder_role_beats_invalidIdentifierType(
        address caller,
        ...
        security().updateCustomMetadata("", value); // empty type triggers InvalidIdentifierType
    }
}
