...
contract B20SecurityAnnounceTest is B20SecurityTest {
    ...
    function test_announce_revert_internalCallMalformed(bytes calldata shortBlob) public {
        ...
        vm.prank(operator);
        vm.expectRevert(abi.encodeWithSelector(IB20Security.InternalCallMalformed.selector, shortBlob));
    ...
    function test_announce_revert_recursion() public {
        ...
        bytes[] memory inner = _singletonBytes(
            abi.encodeWithSelector(IB20Security.announce.selector, new bytes[](0), "inner", "desc", "uri")
        );

        vm.prank(operator);
        vm.expectRevert(IB20Security.AnnouncementInProgress.selector);
