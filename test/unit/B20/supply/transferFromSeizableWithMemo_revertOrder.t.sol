...
/// @title Differential check-order tests for `transferFromSeizableWithMemo`.
///
/// @notice **Canonical order (Solidity reference):**
///         1. PAUSE (`whenNotPaused(SEIZE)` modifier) → `ContractPaused`
///         2. ROLE (`onlyRole(TRANSFER_FROM_SEIZABLE_ROLE)` modifier) → `AccessControlUnauthorizedAccount`
...
contract B20TransferFromSeizableWithMemoRevertOrderTest is B20Test {
    address internal seizer = makeAddr("seizer");

    /// @notice PAUSE beats ROLE.
    function test_transferFromSeizableWithMemo_revertOrder_pause_beats_role(address caller, address from, address to)
        public
    {
        ...
        token.transferFromSeizableWithMemo(from, to, 1, bytes32(0));
    }

    /// @notice ROLE beats ZERO-ACTORS (unauthorized caller reverts before the `to == 0` check).
    function test_transferFromSeizableWithMemo_revertOrder_role_beats_zeroActors(address caller, address from) public {
        _assumeValidCaller(caller);
        _assumeValidActor(from);
        vm.assume(caller != admin);
        ...
        token.transferFromSeizableWithMemo(from, address(0), 1, bytes32(0));
    }

    /// @notice ZERO-ACTORS beats BLOCKED (`to == 0` reverts before the seizable check on `from`).
    function test_transferFromSeizableWithMemo_revertOrder_zeroActors_beats_blocked(address from) public {
        _assumeValidActor(from);
        _grantRole(B20Constants.TRANSFER_FROM_SEIZABLE_ROLE, seizer);
        // SEIZABLE_ACCOUNT_POLICY left at ALWAYS_ALLOW → `from` would be "not blocked", but `to == 0` fires first.
    ...
    function test_transferFromSeizableWithMemo_revertOrder_blocked_beats_balance(address from, address to) public {
        _assumeValidActor(from);
        _assumeValidActor(to);
        vm.assume(from != to);
        _grantRole(B20Constants.TRANSFER_FROM_SEIZABLE_ROLE, seizer);
        // Default SEIZABLE_ACCOUNT_POLICY is ALWAYS_ALLOW → `from` is NOT blocked; zero balance too.
    ...
    function test_transferFromSeizableWithMemo_revertOrder_pause_beats_blocked(address from, address to) public {
        _assumeValidActor(from);
        _assumeValidActor(to);
        vm.assume(from != to);
        _grantRole(B20Constants.TRANSFER_FROM_SEIZABLE_ROLE, seizer);
