...
contract MockB20SlotHelpersTest is B20Test {
    ...
    /// @notice Verifies `seizePolicyIdsSlot()` locates the seize-receiver lane (lane 1).
    /// @dev Write to SEIZE_RECEIVER_POLICY via `updatePolicy`; lane decoder reads back from its own slot.
    function test_seizePolicyIdsSlot_success_decodesReceiverLane() public {
        _setPolicy(B20Constants.SEIZE_RECEIVER_POLICY, PolicyRegistryConstants.ALWAYS_BLOCK_ID);

        uint256 packed = uint256(vm.load(address(token), MockB20Storage.seizePolicyIdsSlot()));
        assertEq(
            MockB20Storage.seizeReceiverPolicyId(packed),
            PolicyRegistryConstants.ALWAYS_BLOCK_ID,
            "seizeReceiverPolicyId lane must reflect the policy write"
        );
    }
