...
/// @notice Coverage for the security variant's configurable `decimals` field
...
///      the `MockB20SecurityStorage.decimalsSlot()` paired-slot pattern,
...
contract B20FactoryCreateB20SecurityDecimalsTest is B20FactoryTest {
    ...
    function test_createB20_revert_decimals_justBelowMin(address caller, bytes32 salt) public {
        ...
        IB20Factory.B20SecurityCreateParams memory p = _securityParams("Security Test", "SEC", admin, bad);
        vm.prank(caller);
    ...
    function test_createB20_success_decimals_inRange_fuzz(address caller, bytes32 salt, uint8 decimalsSeed) public {
        ...
        IB20Factory.B20SecurityCreateParams memory p = _securityParams("Security Test", "SEC", admin, good);
        address token = _createSecurity(caller, salt, p, new bytes[](0));
    ...
    ///         (`MockB20SecurityStorage.decimalsSlot()`) so the storage value
    ...
    function test_createB20_success_decimals_storageRoundTrip(address caller, bytes32 salt, uint8 decimalsSeed) public {
        ...
        assertEq(
            uint256(vm.load(token, MockB20SecurityStorage.decimalsSlot())),
            uint256(good),
        ...
        public
    {
        ...
        emit IB20Factory.B20Created(predicted, IB20Factory.B20Variant.SECURITY, "Security Test", "SEC", good, bytes(""));
