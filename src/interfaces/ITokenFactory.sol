...
///         the broader `IAssetToken` surface (announcements, share
///         ratio, create / adminMint / adminBurn, extra metadatas).
...
///         Returns `DEFAULT` for any `0xB200...` token and `ASSET` for
...
interface ITokenFactory {
    ...
    ///         tokens.
    function ASSET_ADDRESS_PREFIX() external view returns (bytes2);
    ...
    ///         Security tokens implement the broader `IAssetToken`
    ///         surface.
    ///
    ...
    ///         address derived from `(ASSET prefix, msg.sender, params.salt)`.
    ///         No bootstrap mint; asset tokens use `create`
