...
interface IB20 {
    ...
    /// @dev Reverts with `InvalidReceiver` when `to == address(0)` or `from == to` (a self-seize is a
    ///      no-op move that would otherwise still emit a misleading `Transfer`/`Memo`/`Seized`).
