...
- A non-zero destination that is not the holder and not the token (typically a treasury).
...
| `InvalidReceiver(to)`                                  | `to` is `address(0)`, the token's own address, or `from == to`.                                                             | Use a distinct, non-zero safekeeping address that is not the token.                |
...
A balance already sitting at the token address can still be recovered. `seizeWithMemo(token, treasury, amount, memo)` is allowed; seizing *to* the token is not.
