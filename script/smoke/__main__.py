...
Journeys: factory, asset, multiplier, stablecoin, seize, policy, policy-invert, invariants — or
`all` to run every journey in sequence. Env (RPC_URL / DEPLOYER_PK / USER2_PK) is sourced by the
Makefile from .env; running directly requires it exported. A preflight liveness probe checks the
b20 precompiles are actually active on the target chain (fork >= Beryl); if not, the journey is
skipped rather than reporting environment state (inactive feature → account-state fall-through) as
contract defects.
...
JOURNEYS = {
    ...
    "policy-invert": "smoke.journeys.policy_invert",
